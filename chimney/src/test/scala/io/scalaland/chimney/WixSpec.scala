  ...
  val tests = Tests {
    ...
    "Dsl" - {
      ...
      "supports scala enumerations and java enums" - {
        ...
        "transforming enumeration into enumeration with customization" - {
          implicit val t = Transformer
            .define[JavaEnums.JavaColors, colors6.Color.Color]
            .withCoproductValue(JavaEnums.JavaColors.Black, colors6.Color.Red) // for now only constants are supported
            .buildTransformer

          t.transform(JavaEnums.JavaColors.Black) ==> colors6.Color.Red
          t.transform(JavaEnums.JavaColors.Red) ==> colors6.Color.Red
        }
    ...
    "TransformerInto allows to override exceptions" - {
      ...
      val transformerInto = new TransformerInto[
        ...
        source,
        new TransformerDefinition(Map.empty, Map.empty, Map.empty),
        mapper
      ).enableUnsafeOption
